...
and per-image detail controls, typed API transport, TanStack Query request lifecycle management,
and connection modes for same-origin
...
The existing dependency-free browser client remains at `/web/` while private-value consent and
admin feature parity are implemented. It uses the streaming run
