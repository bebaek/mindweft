...
| `MINIGENT_TENANT_EXECUTION_CONFIGS` | Optional per-tenant LLM, tool, skill, capability, backend, and quality config. Tenant execution config currently supplies shared resources; the proposed [user execution overlay](docs/user-execution-extensibility.md) adds personal agents, skills, capability profiles, and MCP servers without requiring tenant config edits. |
