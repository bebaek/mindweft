...
function ExecutionConfigEditor({ connections, onTestConnection, onReplaceCredential, platform, current, mcpCatalog, allowCustomMcpServers, mcpCatalogPending, mcpCatalogError, pending, validation, error, onClose, onApply }: { connections: TenantMcpConnection[]; onTestConnection?: (name: string) => Promise<void>; onReplaceCredential?: (serverName: string, token: string) => Promise<void>; platform: boolean; current: Record<string, unknown> | null; mcpCatalog: AdminMcpServerCatalogItem[]; allowCustomMcpServers: boolean; mcpCatalogPending: boolean; mcpCatalogError: string | null; pending: boolean; validation: AdminExecutionValidation | null; error: string | null; onClose: () => void; onApply: (config: Record<string, unknown>) => void }) {
  ...
  return <dialog ref={dialogRef} className="admin-dialog execution-config-dialog" aria-labelledby="execution-editor-title" onCancel={(event) => { event.preventDefault(); requestClose(); }}>
    <form onSubmit={submit} onChange={() => setDirty(true)}>
      <fieldset disabled={credentialSaving} style={{ border: 0, padding: 0, margin: 0, minWidth: 0 }}>
      ...
      {tab === "presets" && <div className="execution-editor-section"><CapabilityMCPAccessEditor capabilities={capabilities} mcpServers={mcpServers} onChange={(value) => { setCapabilities(value); setDirty(true); }} /><AgentPresetEditor agents={agents} capabilities={capabilities} llmProfiles={llmProfiles} onChange={(value) => { setAgents(value); setDirty(true); }} /><details className="execution-advanced"><summary>Advanced preset JSON</summary><JsonField label="Capability profiles" value={capabilities} onChange={setCapabilities} help="JSON object with default_profile and tool-narrowing items." rows={11} /><JsonField label="Agent presets" value={agents} onChange={setAgents} help="JSON object whose items combine skills and capability profiles." rows={11} /></details></div>}
      {tab === "advanced" && <div className="execution-editor-section"><label className="execution-text-field">Default LLM profile<input value={defaultLlmProfile} onChange={(event) => setDefaultLlmProfile(event.target.value)} placeholder="Optional profile name" /></label><OAuthProfileBindings value={llmProfiles} onChange={(value) => { setLlmProfiles(value); setDirty(true); }} /><JsonField label="LLM profiles" value={llmProfiles} onChange={setLlmProfiles} help="JSON object of named LLM configurations." rows={12} /><JsonField label="Other top-level configuration" value={topLevelAdvanced} onChange={setTopLevelAdvanced} help="JSON object for supported fields not represented in the guided sections." /></div>}
