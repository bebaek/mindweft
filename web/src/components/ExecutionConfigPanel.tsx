...
function ExecutionConfigEditor({ current, pending, validation, error, onClose, onApply }: { current: Record<string, unknown> | null; pending: boolean; validation: AdminExecutionValidation | null; error: string | null; onClose: () => void; onApply: (config: Record<string, unknown>) => void }) {
  ...
  return <dialog ref={dialogRef} className="admin-dialog execution-config-dialog" aria-labelledby="execution-editor-title" onCancel={(event) => { event.preventDefault(); requestClose(); }}>
    <form onSubmit={submit} onChangeCapture={() => setDirty(true)}>
      ...
      {tab === "llm" && <div className="execution-editor-section"><div className="execution-field-grid"><label>Provider<input required value={provider} onChange={(event) => setProvider(event.target.value)} placeholder="mock" /></label><label>Model<input value={model} onChange={(event) => setModel(event.target.value)} placeholder="Provider default" /></label><label className="wide">Base URL<input type="url" value={baseUrl} onChange={(event) => setBaseUrl(event.target.value)} placeholder="Provider default" /></label><label>Timeout (seconds)<input required type="number" min="0.1" step="0.1" value={timeout} onChange={(event) => setTimeout(event.target.value)} /></label><label>API key<input type="password" autoComplete="off" value={apiKey} disabled={clearApiKey} onChange={(event) => setApiKey(event.target.value)} placeholder={initial.hadApiKey ? "Stored secret — leave blank to preserve" : "Optional"} /></label>{initial.hadApiKey && <label className="execution-checkbox"><input type="checkbox" checked={clearApiKey} onChange={(event) => setClearApiKey(event.target.checked)} />Clear stored API key</label>}</div><JsonField label="Advanced LLM settings" value={llmAdvanced} onChange={setLlmAdvanced} help="JSON object for headers, token limits, thinking, caching, and input modalities." /></div>}
      {tab === "tools" && <div className="execution-editor-section"><label className="execution-text-field">Allowed local tools<input value={localTools} onChange={(event) => setLocalTools(event.target.value)} placeholder="echo, current_time, calculator" /><small>Comma-separated. Leave blank to use the runtime default tool policy.</small></label><JsonField label="MCP servers" value={mcpServers} onChange={setMcpServers} help="JSON array of MCP server definitions. Redacted header values preserve their stored secrets." rows={10} /><JsonField label="Advanced tool policy" value={toolsAdvanced} onChange={setToolsAdvanced} help="JSON object for result redaction and other tool policy fields." /></div>}
