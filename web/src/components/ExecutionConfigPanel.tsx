...
function ExecutionConfigEditor({ platform, current, mcpCatalog, allowCustomMcpServers, mcpCatalogPending, mcpCatalogError, pending, validation, error, onClose, onApply }: { platform: boolean; current: Record<string, unknown> | null; mcpCatalog: AdminMcpServerCatalogItem[]; allowCustomMcpServers: boolean; mcpCatalogPending: boolean; mcpCatalogError: string | null; pending: boolean; validation: AdminExecutionValidation | null; error: string | null; onClose: () => void; onApply: (config: Record<string, unknown>) => void }) {
  ...
  return <dialog ref={dialogRef} className="admin-dialog execution-config-dialog" aria-labelledby="execution-editor-title" onCancel={(event) => { event.preventDefault(); requestClose(); }}>
    <form onSubmit={submit} onChangeCapture={() => setDirty(true)}>
      ...
      {tab === "presets" && <div className="execution-editor-section"><CapabilityMCPAccessEditor capabilities={capabilities} mcpServers={mcpServers} onChange={(value) => { setCapabilities(value); setDirty(true); }} /><JsonField label="Capability profiles" value={capabilities} onChange={setCapabilities} help="JSON object with default_profile and tool-narrowing items." rows={11} /><JsonField label="Agent presets" value={agents} onChange={setAgents} help="JSON object whose items combine skills and capability profiles." rows={11} /></div>}
