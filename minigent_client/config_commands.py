...
def export_local_coding_config(args: argparse.Namespace) -> dict[str, object]:
    ...
    bridge_host = env.get("MINIGENT_CODING_BRIDGE_HOST") or runner.DEFAULT_BRIDGE_HOST
    bridge_name = env.get("MINIGENT_CODING_BRIDGE_NAME") or runner.DEFAULT_BRIDGE_NAME
    bridge_port = int(env.get("MINIGENT_CODING_BRIDGE_PORT") or runner.DEFAULT_BRIDGE_PORT)
    ...
    gateway_path_prefix = env.get("MINIGENT_CODING_MCP_GATEWAY_PATH_PREFIX") or runner.DEFAULT_MCP_GATEWAY_PATH_PREFIX
    ...
    text_bridge_port = int(env.get("MINIGENT_CODING_TEXT_BRIDGE_PORT") or runner.DEFAULT_TEXT_BRIDGE_PORT)
    text_bridge_url = f"http://{bridge_host}:{text_bridge_port}/mcp"
    shell_enabled = runner.env_flag_enabled(env.get("MINIGENT_CODING_SHELL_ENABLED"))
    shell_bridge_name = env.get("MINIGENT_CODING_SHELL_BRIDGE_NAME") or runner.DEFAULT_SHELL_BRIDGE_NAME
    shell_bridge_port = int(env.get("MINIGENT_CODING_SHELL_BRIDGE_PORT") or runner.DEFAULT_SHELL_BRIDGE_PORT)
...
def _sanitize_coding_mcp_server_spec(server: dict[str, Any]) -> dict[str, object]:
    ...
    for key, value in server.items():
        if key == "headers" and isinstance(value, dict):
            ...
            exported[key] = {
                env_key: value if isinstance(value, str) and value.startswith("${") else f"${{{env_key}}}"
