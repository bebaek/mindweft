...
def load_demo_module() -> ModuleType:
    script_path = Path(__file__).resolve().parents[1] / "scripts" / "demo_opencode_backend.py"
    spec = importlib.util.spec_from_file_location("demo_opencode_backend", script_path)
...
def test_demo_opencode_backend_drives_run_endpoint(monkeypatch, capsys) -> None:
