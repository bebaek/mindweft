...
def test_demo_peer_agent_submits_and_polls_through_minigent(
    ...
    def fake_request_json(
        ...
        if method == "GET" and url == "http://minigent.test/peer-agents/pi/agent-card":
            return {"name": "pi-coding-agent"}
        if method == "POST" and url == "http://minigent.test/peer-agents/pi/tasks":
            assert payload == {"cwd": "/workspace/project", "prompt": "summarize"}
            return {"task_id": "task_123", "status": "running"}
        if method == "GET" and url == "http://minigent.test/peer-agents/pi/tasks/task_123":
            return {
                "task_id": "task_123",
                "status": "completed",
                "exit_code": 0,
    ...
    assert calls == [
        ...
        ("GET", "http://minigent.test/peer-agents/pi/agent-card", None),
        (
            "POST",
            "http://minigent.test/peer-agents/pi/tasks",
            {"cwd": "/workspace/project", "prompt": "summarize"},
        ),
        ("GET", "http://minigent.test/peer-agents/pi/tasks/task_123", None),
        ("GET", "http://minigent.test/peer-agents/pi/tasks/task_123/events", None),
    ]
...
def test_demo_peer_agent_can_cancel_after_delay(monkeypatch, capsys) -> None:
    ...
    assert (
        "POST",
        "http://minigent.test/peer-agents/pi/tasks/task_123/cancel",
        None,
    ) in calls
    output = capsys.readouterr().out
