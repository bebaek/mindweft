...
def test_export_and_import_thread_archive(monkeypatch: Any, tmp_path: Path, capsys: Any) -> None:
    ...
    imported_timestamp_policies: list[str] = []
    ...
    def urlopen(request: Any) -> _Response:
        ...
        if "/threads/import?profile_policy=" in request.full_url:
            ...
            timestamp_policy = query["timestamp_policy"][0]
            dry_run = query.get("dry_run") == ["true"]
            imported_policies.append(policy)
            imported_organization_policies.append(organization_policy)
            imported_timestamp_policies.append(timestamp_policy)
            imported_payloads.append(json.loads(request.data.decode("utf-8")))
            return _Response(
                body={
    ...
    assert cli.main(["import", str(archive_path), "--timestamp-policy", "preserve"]) == 0
    assert imported_payloads == [archive, archive, archive, archive, archive]
    assert imported_policies == ["available", "strict", "available", "available", "available"]
    assert imported_organization_policies == ["reset", "reset", "reset", "preserve", "reset"]
    assert imported_timestamp_policies == ["reset", "reset", "reset", "reset", "preserve"]
    captured = capsys.readouterr()
    assert captured.out == (
...
def test_admin_threads_delete_json(monkeypatch: Any, capsys: Any) -> None:
    ...
    lineage_response = {
        **response,
        "deleted_thread_ids": ["thread-root", "thread-1"],
        "deleted_count": 2,
    }
    ...
    def urlopen(request: Any) -> _Response:
        ...
        if request.full_url.endswith(
            "/admin/tenants/tenant-a/threads/thread-1?imported_lineage=true"
        ):
            return _Response(body=lineage_response)
        raise AssertionError(f"Unexpected request: {request.full_url}")
    ...
    assert calls[-1] == (
        "DELETE",
        "http://127.0.0.1:8000/admin/tenants/tenant-a/threads/thread-1?imported_lineage=true",
    )
    assert json.loads(capsys.readouterr().out) == lineage_response
