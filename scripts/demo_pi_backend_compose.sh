...
KEEP_RUNNING=false
PREPARE_PI_HOME="${MINDWEFT_PREPARE_PI_CONTAINER_HOME:-${MINIGENT_PREPARE_PI_CONTAINER_HOME:-true}}"
DEMO_ARGS=()
...
cleanup() {
  local status=$?
  if [[ $status -ne 0 ]]; then
    docker compose -f "$COMPOSE_FILE" logs --tail=120 mindweft local-agent-wrapper >&2 || true
  fi
...
wait_for_mindweft() {
  local url="http://127.0.0.1:$MINDWEFT_PORT/health"
  for _ in {1..120}; do
...
PY
  ...
  echo "Mindweft did not become healthy at $url" >&2
  return 1
}
