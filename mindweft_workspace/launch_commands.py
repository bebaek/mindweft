...
def build_mcp_gateway_command(config_path: Path, host: str, port: int) -> list[str]:
    return [
        sys.executable,
        "-c",
        "from mindweft_workspace.bridge.gateway import main; main()",
        "--config",
        str(config_path),
        "--host",
        host,
...
def build_shell_mcp_server_command(
    ...
    *,
    ...
    return [
        sys.executable,
        "-c",
        "from mindweft_workspace.servers.shell import main; raise SystemExit(main())",
        *workspace_args,
...
def build_text_mcp_server_command(workspaces: Path | list[Path]) -> list[str]:
    ...
    return [
        sys.executable,
        "-c",
        "from mindweft_workspace.servers.text import main; raise SystemExit(main())",
        *workspace_args,
    ]
