...
from mindweft_client.output import StreamProgressRenderer, TokenMode
...
class MindweftAPIClient:
    ...
    def export_thread_archive(self, thread_id: str) -> dict[str, Any]:
        response = self.request_json(
            "GET",
            f"{self._config.base_url}/threads/{thread_id}/archive",
        )
        if not isinstance(response, dict):
            raise RuntimeError("Mindweft thread archive response must be an object")
        return cast(dict[str, Any], response)

    def export_thread_lineage_archive(self, thread_id: str) -> dict[str, Any]:
        response = self.request_json(
            "GET",
            f"{self._config.base_url}/threads/{thread_id}/lineage/archive",
        )
        if not isinstance(response, dict):
    ...
    def import_thread_archive(
        ...
        *,
        ...
        if not isinstance(response, dict):
            raise RuntimeError("Mindweft thread archive import response must be an object")
    ...
    def get_imported_thread_lineage(self, thread_id: str) -> dict[str, Any]:
        response = self.request_json(
            "GET",
            f"{self._config.base_url}/threads/{thread_id}/imported-lineage",
        )
        if not isinstance(response, dict):
            raise RuntimeError("Mindweft imported-lineage response must be an object")
        return cast(dict[str, Any], response)
    ...
    def get_admin_imported_thread_lineage(
        self,
        tenant_id: str,
        thread_id: str,
    ) -> dict[str, Any]:
        response = self.request_json(
            ...
            (
                ...
                f"{thread_id}/imported-lineage"
            ),
        )
        if not isinstance(response, dict):
            raise RuntimeError("Mindweft admin imported-lineage response must be an object")
        return cast(dict[str, Any], response)
    ...
    def fork_thread(self, thread_id: str, *, at_message_id: str) -> dict[str, Any]:
        ...
        forked = cast(dict[str, Any], response)
        forked_thread_id = forked.get("thread_id")
        if isinstance(forked_thread_id, str) and forked_thread_id:
            self._thread_id = forked_thread_id
        return forked
